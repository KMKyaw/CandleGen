...
generator.load_state_dict(torch.load('bullx2_generatorv7_vii_len30.pth', map_location=device))
generator.eval()

quantile_transformer_r1 = load('bullx2_quantile_transformer_r1_len30.joblib')
quantile_transformer_r2 = load('bullx2_quantile_transformer_r2_len30.joblib')
...
all_df.to_csv('synthetic_bullx2_candles_len30.csv')
...
def parse_chunk(chunk):
    if isinstance(chunk, list):
        ...
        print(f"Warning: chunk is None")
        return np.array([])
    else:
        print(f"Unsupported type for chunk: {type(chunk)}, value: {chunk}")
...
# total_items = sum(r['chunk_length'] for r in results)
# print(f"\nTotal number of items in all chunks: {total_items}")
