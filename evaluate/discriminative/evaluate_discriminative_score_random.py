...
for fake_path, orig_path in zip(fake_files, original_files):
    ...
    # Filter original data to only rows with o_chunk length 19
    orig_df = orig_df[orig_df['o_chunk'].apply(lambda x: len(eval(x)) if isinstance(x, str) else len(x) == 19)].reset_index(drop=True)
...
score = custom_discriminative_score_torch(all_original_np, all_fake_np)
