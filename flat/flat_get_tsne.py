...
generator.load_state_dict(torch.load('flat_generatorv7_vii_len10.pth', map_location=device))
generator.eval()

# --- Load quantile transformers ---
quantile_transformer_r1 = load('flat_quantile_transformer_r1_len10.joblib')
quantile_transformer_r2 = load('flat_quantile_transformer_r2_len10.joblib')

df = pd.read_feather('../data/flatv7.feather')
filtered_c = [seq.copy() for seq in np.array(df['c_chunk']) if len(seq) == 10]
...
all_df.to_feather('synthetic_flat_candles.feather')
...
print(f"Number of OHLC violations: {num_violations} out of {len(open_)}")


df = pd.read_feather('../data/flatv7.feather')
df = df[df['o_chunk'].apply(lambda x: len(x) == 10)].reset_index(drop=True)
...
def generate_tsne_and_fid_all_sequences_normalized(generator, normalized_data, latent_dim, device):
    # Load quantile transformers
    quantile_transformer_r1 = load('flat_quantile_transformer_r1.joblib')
    quantile_transformer_r2 = load('flat_quantile_transformer_r2.joblib')
...
# flatten_c_chunks_df = np.concatenate(c_chunks_df)

# plt.figure(figsize=(10, 6))
# plt.hist(all_c_values, bins=50, color='skyblue', edgecolor='black', alpha=0.5)
# plt.hist(flatten_c_chunks_df, bins=50, color='green', edgecolor='black', alpha=0.5)
